...
이 패키지는 chat feature의 HTTP API adapter, WebSocket gateway adapter, usecase orchestration, domain policy, runtime port contract를 함께 소유합니다. app은 process bootstrap과 concrete infra wiring만 맡고, chat command/usecase/handler/session routing 내부 구현은 이 패키지 안에 둡니다.
...
`src/api/**`, `src/gateway/**`, `src/api/usecases/**`, `src/gateway/usecases/**`, `src/api/domain/**` 내부 deep import는 금지합니다. package root와 public subpath만 consumer contract입니다.
