...
export function createApiRuntimeDeps(
  ...
  const defaultTicketHasher: NonNullable<RealtimeChatApiRuntimeDeps["ticketHasher"]> = {
...
export function createGatewayRuntimeDeps(overrides: GatewayRuntimeDepsOverrides = {}): {
  deps: RealtimeChatGatewayRuntimeDeps;
  outboundHandlers: Array<(event: OutboundMessageDeliveryRequested) => void | Promise<void>>;
} {
  let idSequence = 0;
  const outboundHandlers: Array<(event: OutboundMessageDeliveryRequested) => void | Promise<void>> =
    [];
...
export class FakeWebSocketConnection implements WebSocketConnectionLike {
  ...
  private messageHandler?: ((payload: WebSocketMessagePayload) => void | Promise<void>) | undefined;
...
export function rejectedTicket(reason: RealtimeChatErrorCode, message?: string) {
  return {
