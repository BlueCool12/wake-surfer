...
async function sendUserMessage(
  ...
  if (existing) {
    return toAcceptedResponse(command.commandId, existing, command.clientMessageId);
  }
  ...
  try {
    ...
    return toAcceptedResponse(command.commandId, message, command.clientMessageId);
  } catch (error) {
