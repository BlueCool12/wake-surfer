...
| FS-MSG-005 | ACK 유실로 같은 명령 재전송 | C: 같은 optimistic item; G: 새 요청; A: 기존 row 반환; O: 현행 Gateway가 created를 다시 fan-out | DB 중복 없음, wire 중복 있음 | `messageId + streamId + sequence`로 drop. 동일 key의 target/text도 같아야 한다 | dedupe·payload 충돌 검사 구현 |
