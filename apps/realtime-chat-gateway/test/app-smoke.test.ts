...
describe("realtime-chat-gateway app smoke", () => {
  ...
  it("mounted WebSocket path에서 gateway.connected를 보낸다", async () => {
    ...
    try {
      const port = portOf(app.address());
      const socket = new WebSocket(`ws://127.0.0.1:${port}/ws/realtime-chat?ticket=test-ticket`);
...
async function expectJson(url: string): Promise<{ status: number; body: unknown }> {
