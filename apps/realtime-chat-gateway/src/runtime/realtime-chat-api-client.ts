...
export function createHttpRealtimeChatApiClient(apiBaseUrl: string): RealtimeChatApiClientPort {
  ...
  return {
    ...
    sendDMMessage(request) {
      return postJson<MessageCommandResponse>(baseUrl, "/internal/messages/dm", request);
    },
    ...
    markReadCursor(request) {
      return postJson<MarkReadCursorResponse>(baseUrl, "/internal/read-cursors", request);
    },
    syncStreamMessages(request) {
      return getJson<SyncStreamMessagesResponse>(buildSyncMessagesUrl(baseUrl, request));
    },
  };
}

export function createHttpGatewayTicketConsumePort(apiBaseUrl: string): GatewayTicketConsumePort {
  const baseUrl = normalizeBaseUrl(apiBaseUrl);
...
async function postJson<T>(baseUrl: string, suffix: string, body: unknown): Promise<T> {
...
async function readJsonResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    throw new Error(`realtime chat api request failed: ${response.status} ${JSON.stringify(body)}`);
  }

  return body as T;
}

function buildSyncMessagesUrl(baseUrl: string, request: SyncStreamMessagesRequest): string {
  const url = new URL(`${baseUrl}/streams/${encodeURIComponent(request.streamId)}/messages`);
